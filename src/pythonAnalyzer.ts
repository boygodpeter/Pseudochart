...
export function parsePythonWithAST(code: string): Promise<{
    ...
    return new Promise((resolve, reject) => {
        ...
        function tryNextPython() {
            ...
            const args = ['-X', 'utf8', tempScriptPath]; 
...
`;
    ...
    def visit_If(self, node):
        ...
        if node.body:
            ...
            #如果 if body 沒有以return/break結束，保存當前節點
    ...
    def visit_For(self, node):
        ...
        #將迴圈節點加入stack（用於 break/continue)
        self.loop_stack.append(for_id)
        
        #儲存當前狀態
        old_branch_ends = self.branch_ends[:]
        self.branch_ends = []
        break_nodes = []  # 收集屬於這個迴圈的break節點
    ...
    def visit_Break(self, node):
        ...
        #將此節點加入branch_ends以便迴圈處理

        #break節點會在visit_For或visit_While中被收集

        self.branch_ends.append(node_id)
        
        #break會跳出迴圈，所以設置current_node為None
